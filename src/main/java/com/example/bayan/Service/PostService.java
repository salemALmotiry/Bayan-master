...
public class PostService {
    ...
    public void sendPostForOnoBroker(Integer customerId,Integer brokerId, AddPostDTO addPostDTO) {
        ...
        Contract contract = contractRepository.findContractByCustomerIdAndBrokerIdAndStatus(customerId,brokerId,"Active");

        if (contract != null && contract.getNumberOfOrder() >0){
            // Create the offer
            Offer offer = new Offer();
            offer.setPost(post);
            offer.setPrice(contract.getSupPrice());
            offer.setBroker(broker);
            offerRepository.save(offer);

            if (post.getHasDelivery()) {
                offer.setDeliveryIncluded(true);
                offer.setDeliveryPrice(contract.getPriceDelivery());
            } else {
            ...
            contract.setNumberOfOrder(contract.getNumberOfOrder()-1);
            contractRepository.save(contract);

        } else if (contract != null && contract.getNumberOfOrder() == 0 && !contract.getStatus().equals("Expired")) {
    ...
    public List<PostDTO> getAllPostsByCountryOfOrigin(String countryOfOrigin) {
    ...
    public List<PostDTO> getAllPostsByShipmentType(String shipmentType) {
    ...
    public List<PostDTO> getAllPostsByCategoryAndCountryOfOrigin(String category, String countryOfOrigin) {
    ...
    public List<PostDTO> getAllPostsByCategoryAndShipmentType(String category, String shipmentType) {
    ...
    // filter posts by border name
    public List<Post> getAllPostByBorderName(String borderName) {
        List<Post> posts = postRepository.findAllByBorderName(borderName);

        if (posts.isEmpty()) {
            throw new ApiException("No posts found for that border");
        }
        ...
        if (filteredPosts.isEmpty()) {
            throw new ApiException("No posts without brokers found for the specified border");
        }
        return filteredPosts;
    }

    // **All Posts By Category, Shipment Type, And Country Of Origin (Excluding Posts with a Broker)
    public List<PostDTO> getAllPostsByCategoryShipmentTypeAndCountry(String category, String shipmentType, String countryOfOrigin) {
        List<Post> posts = postRepository.findPostByCategoryAndShipmentTypeAndCountryOfOrigin(category, shipmentType, countryOfOrigin);

        if (posts.isEmpty()) {
            throw new ApiException("No posts found for the specified category: " + category +
                    ", shipment type: " + shipmentType + ", or country of origin: " + countryOfOrigin);
        }
