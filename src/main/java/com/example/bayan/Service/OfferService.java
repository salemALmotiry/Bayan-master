...
public class OfferService {
    ...
    public void acceptOffer(Integer userId, Integer offerId) {
        ...
        notification2.setCreateAt(LocalDate.now());
    ...
    public void acceptOfferForSubscriptionPost(Integer userId, Integer offerId) {
        ...
        contract.setNumberOfOrder(post.getShipmentsNumber());
        contract.setSupPrice(offer.getPrice());
        ...
        BigDecimal priceDelivery = contract.getPriceDelivery() != null ? contract.getPriceDelivery() : BigDecimal.ZERO;
        ...
        contract.setPriceDelivery(offer.getDeliveryPrice());
        contract.setStartDate(LocalDate.now());
        contract.setBroker(offer.getBroker());
        contract.setHasDelivery(offer.getDeliveryIncluded());
        contract.setCustomer(customer.getCustomer());
        ...
        String customerEmailText = "<html><body>" +
                ...
                "<li>عدد الشحنات: " + contract.getNumberOfOrder() + "</li>" +
                "<li>السعر لكل شحنة: " + contract.getSupPrice() + " ريال</li>" +
                "<li>السعر الإجمالي: " + contract.getTotalPrice() + " ريال</li>" +
                "<li>تاريخ البدء: " + contract.getStartDate() + "</li>" +
                "<li>حالة العقد: " + contract.getStatus() + "</li>" +
                "</ul>" +
    ...
    public List<com.example.bayan.DTO.OUT.OfferDTO> myOffersAsBroker(Integer brokerId) {
        ...
        // Fetch offers separately for post and subscriptionPost
        List<Offer> postOffers = offerRepository.findActivePostOffersByBroker(broker.getId());
        List<Offer> subscriptionOffers = offerRepository.findActiveSubscriptionOffersByBroker(broker.getId());

        // Merge the two lists
        List<Offer> allOffers = new ArrayList<>();
        allOffers.addAll(postOffers);
        allOffers.addAll(subscriptionOffers);

        // Convert offers to OfferDTO
        List<com.example.bayan.DTO.OUT.OfferDTO> offerDTOS = new ArrayList<>();
        for (Offer offer : allOffers) {
            com.example.bayan.DTO.OUT.OfferDTO offerDTO = new com.example.bayan.DTO.OUT.OfferDTO();
            offerDTO.setOfferStatus(offer.getOfferStatus());
            offerDTO.setPrice(offer.getPrice());
            offerDTO.setDeliveryIncluded(offer.getDeliveryIncluded());

            if (offer.getPost() != null) {
                // Populate fields for `post_id` offers
                offerDTO.setProductCategory(offer.getPost().getCategory());
                offerDTO.setBorderName(offer.getPost().getBorder().getName());

            } else if (offer.getSubscriptionPost() != null) {
                // Populate fields for `subscription_post_id` offers
                offerDTO.setProductCategory(offer.getSubscriptionPost().getProductCategory());
                offerDTO.setBorderName(offer.getSubscriptionPost().getBorder().getName());
