...
public class AuthService {
    ...
    public List<CustomBrokerForAdminDTO> getAllCustomBroker(Integer adminId) {
        ...
        for (CustomsBroker c : customsBrokers) {
            ...
            List<String> borderNames = c.getBorders().stream()
                    .map(Border::getName)
                    .collect(Collectors.toList());
            cDto.setBorders(borderNames);
    ...
    public List<CustomBrokerForAdminDTO> getBrokersWaitingForAcceptance(Integer adminId) {
        MyUser admin= authRepository.findMyUserById(adminId);
        if (!admin.getRole().equalsIgnoreCase("ADMIN")){
            throw new ApiException("Sorry,you can't");
        }
        List<CustomsBroker> customsBrokers = customBrokerRepository.findAllByIsActiveFalse();
        List<CustomBrokerForAdminDTO> brokerDTOS = new ArrayList<>();
